...
PATH = ["matrix_list100.txt", "matrix_list200.txt", "matrix_list400.txt", "matrix_list1000.txt"]
...
def compare_matrix(matrix1, matrix2, size):
    for i in range(size):
        for j in range(size):
...
if __name__ == "__main__":
    ...
    for i in range(len(PATH)):
        ...
        for j in range(len(res_check)):
            if not compare_matrix(res_check, arr_res, SIZES[i]):
                check_flag = False
    if check_flag:
